...
func setup(data: EnemyData) -> void:
	...
	var health_mult = 1.0 + (loop * health_scaling)
	var stat_mult = 1.0 + (loop * stat_scaling)
...
func take_damage(amount: int) -> int:
	...
	print("🛡️ Absorbed %d, took %d damage! HP: %d/%d" % [amount - damage, damage, current_health, max_health])
	return damage
